...
img: ../img/droneDrop/thumb.png # Add image post (optional)
...
1. Develop a lightweight payload release system that can be triggered remotely.
2. Control a servo motor to open a hook and release the payload.
3. Build a lightweight housing for the electronics using 3D printing.
...
- **Actuator:** Servo motor controlled via custom PWM signals.
...
The software consists of a control program written in C++ for the ESP32, with custom servo drivers for the SG90 to optimize timing and motion accuracy so the hook would release successfully.
- **Servo Control:** The servo was driven using custom PWM signals generated by the ESP32. The implementation allowed for precise control of the angle and release timing. [View ESP32Servo Code](#).
![](../assets/img/droneDrop/servo.png)
- **Light Detection:** The photoresistor's output was continuously monitored. When the light intensity crossed a predefined threshold I determiend through bench testing, the servo was actuated.
...
### Testing and Results
#### Egg Drop Test
To validate the mechanism's reliability, a series of tests were conducted using an egg as the payload. The drone successfully delivered the payload with precision, and had no accidental releases.
...
#### Future Improvements
- **Reduce Weight:** Maybe designing two separate modules for the microcontroller and servo would reduce the package size on the bottom of the drone, and better distribute thee wight of the system. 
...
This project highlights the intersection of embedded systems and practical engineering, showcasing how simple components can achieve sophisticated results when combined thoughtfully. The payload delivery mechanism paves the way for further exploration into drone automation and robotics.